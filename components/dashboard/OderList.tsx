...
const OderList = () => {
  ...
  const fetchUsers = useCallback(async () => {
    setIsLoading(true);
    try {
      const result = await getAllBoatOrderList(
        debouncedSearch,
        currentPage,
        limit
      );
      ...
      if (result.ok && result.data) {
        setClientData(result?.data?.orders);
          ...
          <table className="w-full text-left rtl:text-right text-gray-500 ">
            ...
            <tbody>
              {clientData?.map((el: any, index: number) => (
                ...
                >
                  <td className="text-left px-4">{el?.user?.name}</td>
                  <td className="text-center">{el?.boatDetails.boatModel}</td>
                  <td className="text-center">{el?.paymentStatus}</td>
                  <td className="text-center">{el?.orderStatus}</td>

                  <td className="flex justify-center space-x-3 py-2">
                    <Link
                      href={`/orderView/${el._id}`}
                      className="bg-green-100 p-1.5 rounded hover:bg-green-200"
                    >
